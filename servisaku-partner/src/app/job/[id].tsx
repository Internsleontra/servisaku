...
import { api, breakdownLines, type BookingDetail, type ServiceDetail } from '@/api/client';
...
export default function JobDetail() {
  ...
  const answerRows = useMemo(() => {
    ...
    return fromQ.length ? fromQ : answersFromBreakdown(breakdownLines(booking?.price_breakdown));
  }, [service, booking]);
  ...
  const lines = breakdownLines(booking.price_breakdown).filter((l) => Number(l.amount) !== 0);
