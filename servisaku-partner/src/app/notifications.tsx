...
// Type → icon + colour + label. Mirrors the ServisAku web NotificationCenter
// (shared by consumer + partner) so the mobile center reads identically.
...
// Calendar-style bucket label (Today / Yesterday / weekday / full date).
...
// Where a notification navigates on tap. Partner job route is /job/[id]; the
// backend emits /jobs/:id, so prefer the explicit booking_id and only navigate
// to routes that exist in the partner app.
function targetFor(n: AppNotification): string | null {
  if (n.booking_id) return `/job/${n.booking_id}`;
  ...
  if (!user) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg }}>
        <Header insets={insets} unreadCount={0} filterActive={false} showMarkAll={false} showFilter={false}
          onBack={() => router.back()} onMarkAll={() => {}} onToggleFilter={() => {}} />
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      {q.isLoading ? (
        ...
        <EmptyState
          ...
          subtitle={search || filter !== 'all' ? 'Try adjusting your search or filter' : 'New job requests and updates will appear here'}
        />
      ) : (
        <ScrollView
