      ...
      {!data ? <div className="px-5 space-y-3">{[0, 1, 2].map(i => <div key={i} className="h-24 bg-surface rounded-2xl animate-pulse" />)}</div> : (
        <div className="px-5 space-y-4 pb-10">
          <div className="bg-surface shadow-[inset_0_0_0_1px_rgb(var(--hairline))] rounded-2xl p-5 flex flex-col items-center gap-1">
            ...
            <p className="text-xs text-ink-tertiary">{t('Your average rating given')}</p>
          </div>
          <div className="flex gap-2">
            {[['given', `${t('Given')} (${data.given.length})`], ['pending', `${t('Pending')} (${data.pending.length})`]].map(([k, l]) => (
          ...
          {tab === 'given' ? (data.given.length ? data.given.map(g => (
            <div key={g.id} className="bg-surface shadow-[inset_0_0_0_1px_rgb(var(--hairline))] rounded-2xl p-4 space-y-2">
              <div className="flex justify-between items-start">
                ...
                {g.anonymous && <span className="text-[9px] bg-raised text-ink-secondary px-2 py-0.5 rounded-full font-semibold">{t('Anonymous')}</span>}
              </div>
              <Stars n={g.rating} />
              <p className="text-sm text-ink-secondary">{g.comment}</p>
              <div className="flex gap-4 pt-1">
                <button onClick={() => toast.info(t('Edit review — coming soon'))} className="flex items-center gap-1.5 text-xs font-semibold text-ink-secondary"><Pencil className="h-3.5 w-3.5" /> {t('Edit')}</button>
                <button onClick={() => del(g.id)} className="flex items-center gap-1.5 text-xs font-semibold text-danger"><Trash2 className="h-3.5 w-3.5" /> {t('Delete')}</button>
              </div>
            </div>
          )) : <div className="text-center py-12 text-ink-secondary text-sm">{t('No reviews yet')}</div>) : (
            data.pending.length ? data.pending.map(p => (
              <div key={p.bookingId} className="bg-surface shadow-[inset_0_0_0_1px_rgb(var(--hairline))] rounded-2xl p-4 space-y-2.5">
                ...
                <button onClick={() => navigate(`/review/${p.bookingId}`)} className="w-full h-10 rounded-lg bg-brand text-white text-sm font-semibold">{t('Write a review')}</button>
              </div>
            )) : <div className="text-center py-12 text-ink-secondary text-sm">{t('All caught up 🎉')}</div>
          )}
        </div>
      )}
    </div>
