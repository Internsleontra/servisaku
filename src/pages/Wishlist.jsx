  ...
  return (
    <div className="font-inter min-h-screen bg-bg max-w-2xl mx-auto">
      <div className="flex items-center gap-3 px-5 pt-12 pb-3">
        ...
        <h1 className="text-xl font-semibold flex-1">Wishlist</h1>
      ...
      {!data ? <div className="px-5 grid grid-cols-2 gap-3">{[0, 1, 2, 3].map(i => <div key={i} className="h-40 bg-surface rounded-2xl animate-pulse" />)}</div> : (
        <div className="px-5 space-y-6 pb-10">
          {data.services.length > 0 && (
            <div>
              <p className="text-[11px] font-semibold text-ink-tertiary mb-2">SAVED SERVICES</p>
                  ...
                  <div key={s.slug} className={`bg-surface shadow-[inset_0_0_0_1px_rgb(var(--hairline))] rounded-2xl overflow-hidden ${view === 'list' ? 'flex' : ''}`}>
                    ...
                    <div className="p-3 flex-1">
                      ...
                      <div className="flex items-center justify-between mt-1.5">
                        ...
                        <button onClick={() => navigate(`/book-service/${s.slug}`)} className="text-[11px] font-semibold bg-brand text-white rounded-lg px-2.5 py-1">Book</button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {data.partners.length > 0 && (
            <div>
              <p className="text-[11px] font-semibold text-ink-tertiary mb-2">FAVOURITE PROS</p>
          ...
          {data.categories.length > 0 && (
            <div>
              <p className="text-[11px] font-semibold text-ink-tertiary mb-2">SAVED CATEGORIES</p>
          ...
          {data.recentlyViewed.length > 0 && (
            <div>
              <p className="text-[11px] font-semibold text-ink-tertiary mb-2">RECENTLY VIEWED</p>
              <div className="flex flex-wrap gap-2">
