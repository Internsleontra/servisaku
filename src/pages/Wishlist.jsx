...
export default function Wishlist() {
  ...
  const remove = (slug) => { setData(d => ({ ...d, services: d.services.filter(s => s.slug !== slug) })); toast.success('Removed from wishlist'); };
  ...
  return (
    <div className="font-inter min-h-screen bg-bg max-w-2xl mx-auto">
      <div className="flex items-center gap-3 px-5 pt-12 pb-3">
        ...
        <h1 className="text-xl font-bold flex-1">Wishlist</h1>
      ...
      {!data ? <div className="px-5 grid grid-cols-2 gap-3">{[0, 1, 2, 3].map(i => <div key={i} className="h-40 bg-surface rounded-2xl animate-pulse" />)}</div> : (
        <div className="px-5 space-y-6 pb-10">
          {data.services.length > 0 && (
            <div>
              <p className="text-[11px] font-bold text-ink-tertiary mb-2">SAVED SERVICES</p>
              <div className={view === 'grid' ? 'grid grid-cols-2 gap-3' : 'space-y-3'}>
                {data.services.map(s => (
                  <div key={s.slug} className={`bg-surface border border-hairline/10 rounded-2xl overflow-hidden ${view === 'list' ? 'flex' : ''}`}>
                    ...
                    <div className="p-3 flex-1">
                      ...
                      <div className="flex items-center justify-between mt-1.5">
                        <span className="text-sm font-bold text-brand">from {formatMYR(s.price)}</span>
                        <button onClick={() => navigate(`/book-service/${s.slug}`)} className="text-[11px] font-bold bg-brand text-white rounded-lg px-2.5 py-1">Book</button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {data.partners.length > 0 && (
            <div>
              <p className="text-[11px] font-bold text-ink-tertiary mb-2">FAVOURITE PROS</p>
              <div className="bg-surface border border-hairline/10 rounded-2xl divide-y divide-hairline/10">
                {data.partners.map(p => (
                  <div key={p.id} className="flex items-center gap-3 px-4 py-3">
                    <div className="w-10 h-10 rounded-full bg-brand-tint flex items-center justify-center font-bold text-brand">{p.name[0]}</div>
          ...
          {data.categories.length > 0 && (
            <div>
              <p className="text-[11px] font-bold text-ink-tertiary mb-2">SAVED CATEGORIES</p>
              <div className="flex flex-wrap gap-2">
                {data.categories.map(c => <button key={c.slug} onClick={() => navigate(`/catalog/${c.slug}`)} className="text-xs font-semibold bg-surface border border-hairline/20 rounded-full px-3.5 py-2">{c.name}</button>)}
              </div>
            </div>
          )}
          {data.recentlyViewed.length > 0 && (
            <div>
              <p className="text-[11px] font-bold text-ink-tertiary mb-2">RECENTLY VIEWED</p>
              <div className="flex flex-wrap gap-2">
                {data.recentlyViewed.map(r => <button key={r.slug} onClick={() => navigate(`/book-service/${r.slug}`)} className="text-xs font-semibold bg-surface border border-hairline/20 rounded-full px-3.5 py-2">{r.name}</button>)}
              </div>
            </div>
          )}
        </div>
