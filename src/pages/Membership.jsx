        ...
        <div className="px-5 space-y-4 pb-10">
          <div className="bg-brand text-white rounded-3xl p-6 shadow-e2">
            ...
            <div className="bg-white/15 rounded-2xl p-4 mt-4">
              <p className="text-[11px] font-semibold text-white/70">LIFETIME SAVINGS</p>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[['Free cancels', m.stats.freeCancellationsLeft], ['Priority used', m.stats.priorityUsed], ['Discounts', m.stats.exclusiveDiscounts]].map(([l, v]) => (
              <div key={l} className="bg-surface shadow-[inset_0_0_0_1px_rgb(var(--hairline))] rounded-2xl p-4 text-center">
                <p className="text-xl font-semibold">{v}</p><p className="text-[10px] text-ink-tertiary mt-1">{l}</p>
              </div>
            ))}
          </div>
          <div className="bg-surface shadow-[inset_0_0_0_1px_rgb(var(--hairline))] rounded-2xl p-4">
            <p className="text-[11px] font-semibold text-ink-tertiary mb-3">YOUR BENEFITS</p>
          ...
          <button onClick={() => toast.info('Membership will end at period close')} className="w-full h-11 rounded-xl shadow-[inset_0_0_0_1px_rgb(var(--hairline))] text-ink-secondary font-semibold">Cancel membership</button>
        </div>
      )}
    </div>
  );
