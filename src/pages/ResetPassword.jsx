...
export default function ResetPassword() {
  ...
  const handleReset = async () => {
    if (!token) { toast.error(t('This reset link is invalid.')); return; }
    if (password.length < 8) { toast.error(t('Password must be at least 8 characters')); return; }
    if (password !== confirm) { toast.error(t('Passwords do not match')); return; }
    setLoading(true);
    try {
      await servisaku.auth.resetPassword(token, password);
      if (checkUserAuth) await checkUserAuth();
      setDone(true);
      toast.success(t('Password updated! You are now signed in.'));
      ...
      <div className="w-full max-w-md rounded-card bg-surface p-6 shadow-e3 md:p-8">
        ...
        {done ? (
          <div className="bg-surface rounded-2xl border border-hairline/40 p-8 text-center">
            ...
            <h1 className="text-2xl font-display font-semibold text-ink">{t('Password updated')}</h1>
            <p className="text-ink-secondary mt-2">{t('Redirecting you in…')}</p>
          </div>
        ) : (
          <div className="bg-surface rounded-2xl border border-hairline/40 p-8">
            <h1 className="text-3xl font-display font-semibold mb-2 text-ink">{t('Set a new password')}</h1>
            <p className="text-ink-secondary mb-8">{t("Choose a strong password you don't use elsewhere.")}</p>

            {!token && (
              <div className="bg-danger-tint border border-danger/30 rounded-xl p-4 text-sm text-danger mb-6">{t('This reset link is missing or invalid. Please request a new one from the login page.')}</div>
            )}

            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-ink pl-1">{t('New password')}</label>
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-ink-tertiary" />
                  <input type={show ? 'text' : 'password'} placeholder="••••••••" aria-label={t('New password')} value={password} onChange={e => setPassword(e.target.value)}
                  ...
                  <button onClick={() => setShow(!show)} aria-label={t('Toggle password visibility')}
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-ink pl-1">{t('Confirm password')}</label>
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-ink-tertiary" />
                  <input type={show ? 'text' : 'password'} placeholder="••••••••" aria-label={t('New password')} value={confirm} onChange={e => setConfirm(e.target.value)}
              ...
              <Button onClick={handleReset} disabled={loading || !token || !password || !confirm}
                ...
                {loading ? t('Updating…') : t('Update password')} <ArrowRight className="h-5 w-5 ml-2" />
              </Button>

              <div className="text-center pt-2">
                <Link to="/otp-login" className="text-sm font-semibold text-brand hover:underline">{t('Back to sign in')}</Link>
              </div>
            </div>
          </div>
        )}
